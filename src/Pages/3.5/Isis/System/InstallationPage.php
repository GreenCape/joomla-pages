...
class InstallationPage extends AdminPage
{
	...
	public function setField($label, $value)
	{
		switch ($label)
		{
			...
			case 'Confirm Admin Password':
				...
				break;
			default:
				throw new \Facebook\WebDriver\Exception\NoSuchElementException("No element found with label '$label'");
		}
	...
	public function setDatabaseType($value)
	{
		...
		$this->driver->findElement(By::xpath("//div[@id='jform_db_type_chzn']//ul[@class='chzn-results']/li[contains(translate(.,'" . strtoupper($value) . "', '" . strtolower($value) . "'), '" . strtolower($value) . "')]"))
		             ->click();
